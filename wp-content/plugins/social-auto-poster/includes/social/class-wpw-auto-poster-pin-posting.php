...
//            $post_featured_img['src'] = "https://i.pinimg.com/736x/52/1f/87/521f873537efc8f6a1008f59586c9086--homemade-cat-toys-diy-cat-post-homemade.jpg";
            ...
            $aa=get_post_meta($post->ID,"fifu_image_url",true);
$img_src=$aa;
if(!is_dir("fmd_cache"))mkdir("fmd_cache");
$ux=explode(".",$aa);
$uz=end($ux);
 $do="fmd_cache/cbyfmd_".md5($aa).".$uz";
...
$ch = curl_init(str_replace(" ","%20",$aa));
            ...
            if (!empty($post_to_users)) {
                ...
                foreach ($post_to_users as $post_to) {
                    ...
                    try {
                        ...
                        sap_add_notice( sprintf( "cbyfmd test ".__('Pinterest: Something was wrong while posting %s', 'wpwautoposter' ), $e->getMessage() ), 'error');
                        $postflg = false;
                    }
                }
            }
